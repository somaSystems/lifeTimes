...
#' leadLagCorr_diffs
#'
#' @description function to calculate median difference (asymmetry)
#' in the mean of lead and lagging time points for different variables
#'
#' @importFrom magrittr %>%
#' @importFrom tidyr pivot_wider
#' @importFrom dplyr left_join arrange group_by summarise
#'
#' @param instanceOf_outputCCFdata_withMetaData output from clusterCCFs, which
#' a function called during the lifeTimesChain function workflow
#'
#' @export
#'
#'
...
dplyr::arrange(an_CCF_ObjectID,an_CCF_Feature, Treatment,anCCF_LAG,)%>%
dplyr::group_by(an_CCF_ObjectID, an_CCF_Feature,Treatment)
...
dplyr::arrange(an_CCF_ObjectID,an_CCF_Feature, Treatment,anCCF_LAG)%>%
dplyr::group_by(an_CCF_ObjectID, an_CCF_Feature,Treatment, lagRange) %>%
...
tidyr::pivot_wider(meanLagRange_outputCCFdata_withMetaData,
...
join_meanLagRange_outputCCFdata_withMetaData <- dplyr::left_join( #join prior to post ratio back to dataframe
...
dplyr::group_by(an_CCF_Feature,Treatment)%>%
...
dplyr::left_join(join_meanLagRange_outputCCFdata_withMetaData,
