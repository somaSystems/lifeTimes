...
lts_TNF <- read.csv("lts_RELAdata.csv")
...
lts_pipeTNF <- lifeTimes::lts_input(.tsData = lts_TNF,
                     .time = "Time_min",
                     .compare_categorical = "oneCategory",
                     .plot_measured_variables = TRUE,
                     .pairedComparisons = lts_RELApairs,
                     .uniqueID_colname ="posIDnew",
                     .metaData = NULL)

wide <- lifeTimes:::lts_tsToWide(lts_pipeTNF) ## can add an argument to remove NA


ccf <- lifeTimes:::lts_wide_ts_to_ccf(.lts_cast_ts = wide, .lts_variables = lts_pipeTNF) # gives an error if there are na

ccf_df <- lifeTimes:::lts_ccf_df(.lts_ccflist = ccf,.lts_variables = lts_pipeTNF)

# meta_df <- lts_TEST_metaData_ccf_join(.lts_dfccf = ccf_df, .lts_variables = lts)

meta_df <- lifeTimes:::lts_metaData_ccf_join(.lts_dfccf = ccf_df, .lts_variables = lts_pipeTNF)

# ccf_summs <-lts_TEST_summarise_ccf(.lts_ccfWithMetaData = meta_df,.lts_variables = lts_pipeTNF)

ccf_summs <- lifeTimes:::lts_summarise_ccf(.lts_ccfWithMetaData = meta_df,.lts_variables = lts_pipeTNF)

# ccf_summs$lts_ccf_summaries

clust_summs <- lifeTimes:::lts_cluster_ccf_summs(.lts_ccf_with_summs =ccf_summs ,.lts_variables = lts_pipeTNF)
