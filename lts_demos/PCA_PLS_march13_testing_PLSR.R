...
# lts_plot_ClustSum(lts_cluster)
#
# lts_plot_coupled(lts_cluster, .lts_facet_by = "cat2", .lts_colour_by = "cat1")
# #
#
#
# ltsC <- lts_calc()
...
library(tidyr)
#look at summary stats functions
colnames(singleSums)
...
w_singleSums <- pivot_wider(singleSums, #make wider so one row per kry num
...
colnarm_w_singleSums <- w_singleSums[,colSums(is.na(w_singleSums))<nrow(w_singleSums)] #remove na

dim(w_singleSums)
dim(colnarm_w_singleSums)

#remove rows where some are na

# install.packages("dplyr")
rownarm_colnarm_w_singleSums <- colnarm_w_singleSums %>% #remove rtows where NA
  tidyr::drop_na()
...
label_w_singleSums <- rownarm_colnarm_w_singleSums[ , !num_cols] #subset label cols
label_w_singleSums
# label_w_singleSums$combineCat <- paste0(label_w_singleSums[,.lts_compare_by[[1]]],label_w_singleSums[,.lts_compare_by[[2]]])
...
#make sumary statistics o original data
...
#make a function for prcomp that incorporates above
#take some summary data
...
### functionalise this to join original and new into summary stats
names.use <- names(w_singleSums)[!(names(w_singleSums) %in% .lts_compare_by)]

w_singleSums_noCategories <- w_singleSums[, names.use ]

join_original_ccf_summ <- merge(sumOriginal,w_singleSums_noCategories, by = c(.lts_unique_ID_colnames))


# join_original_ccf_summ <- merge(sumOriginal, w_singleSums, by = c(.lts_unique_ID_colnames)) # make so joins dont duplicate catchment
...
lts_prcomp_list <- lapply(lts_prcomps, lts_calc_prcomp)
...
lts_PLSR <- lts_calc_prcomp(join_original_ccf_summ)
#Get column names of predictors
lts_predictors <- lts_PLSR$lts_values_pc
lts_response <- as.numeric(lts_PLSR$lts_labels_pc$catchmentRegion.x)
