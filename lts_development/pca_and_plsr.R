...
### GENERATES PCA OF ORIGINAL ANALYSIS #########
...
# valsFrom <- which(!(colnames(singleSums) %in% c(.lts_compare_by,.lts_unique_ID_colnames))) #columns that have measures
...
colnarm_w_singleSums <- w_singleSums[,colSums(is.na(w_singleSums))<nrow(w_singleSums)] #remove columns that are NA

rownarm_colnarm_w_singleSums <- colnarm_w_singleSums %>% #remove rows with NAs
  tidyr::drop_na()
...
label_w_singleSums <- rownarm_colnarm_w_singleSums[ , !num_cols]  #subset label columns of data
# label_w_singleSums
label_w_singleSums$catchSeas <- paste0(label_w_singleSums$season,label_w_singleSums$catchmentRegion) #make compound label



# pc_num_w_singleSums <- prcomp(num_w_singleSums, scale. = TRUE, center = TRUE) #create PC plot
...
#calculate summaries of original data
...
library(ggfortify)
  ...
  summary(catch_model)
  plot(RMSEP(catch_model), legendpos = "topright")
  RMSEP(catch_model)
