...
lts_singleton_modeMaxCorrLAG <- lts_metadf %>%
...
lts_catGroups_modeMaxCorrLAG <- lts_metadf %>%
  # dplyr::select(theLAG, theCCF, !!rlang::sym(lts$lts_uniqueID_colname))%>% #group by unique ID
  ...
  dplyr::group_by(!!rlang::sym(.lts_compare_by[[1]]), #group by categorical variables
                  !!rlang::sym(.lts_compare_by[[2]])) %>%
  # dplyr::ungroup()%>% #ungroup to just get the Mode LAG of everything
  dplyr::mutate(catGroup_ModeLAGatMax = Mode(LAGatMaxCCF), #get mode LAG with Max
                catGroup_mean_corr_atModeLAG = mean(maxCCF)) #get mean corr at mode Lag

lts_clusterOutput <- list(lts_singletonSummary_metadata = lts_singletonSummary_metadata,
                          lts_catGroupsSummary = lts_catGroupsSummary,
                          lts_summ_perLAG = lts_summ_perLAG,
                          lts_singleton_modeMaxCorrLAG = lts_singleton_modeMaxCorrLAG,
                          lts_catGroups_summ_perLAG = lts_catGroups_summ_perLAG,
                          lts_catGroups_modeMaxCorrLAG = lts_catGroups_modeMaxCorrLAG)

                          modeMaxCorrLAG = modeMaxCorrLAG) #this list output includes the matrix of values (mCCF) at chosen lag for dendrogram creation in plotting steps, an alternative is
