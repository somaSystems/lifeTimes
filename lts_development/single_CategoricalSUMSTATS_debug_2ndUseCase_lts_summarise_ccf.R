...
# lts_summarise_ccf <-
  # function(
    ...
    # ,
                              ...
                              .lts_portion = 0.25
                              # )
    # { #for secondary set of summary stats
...
.lts_compare_by <- .lts_variables$lts_compare_by #hotfix move these after IF
.lts_uniqueID_colname <- .lts_variables$lts_uniqueID_colname #hotfix move these after IF
...
lts_summ_lag_range_diffed <- lts_summ_lag_range %>%
  ...
  dplyr::select(where(is.numeric))%>% #make sure not to select unique ID
...
colnames(lts_singletonSummary)

###NOW JOIN categorical variables and Make group summary stats
unq_compareBy <-
  unique(.lts_ccfWithMetaData[, c(.lts_variables$lts_compare_by, #hotfix feb 20 2022, use ccfs instead of original data so can have features
...
lts_singleton_summ_metadata <-  merge(lts_singletonSummary,
  unq_compareBy,
  by = c(.lts_variables$lts_uniqueID_colname))  #categorical variables

colnames(lts_singletonSummary)
colnames(unq_compareBy)
###OUTPUT ONE
